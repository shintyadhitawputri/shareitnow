...
class MainViewModelTest{
    ...
    fun `when Get Story Should Not Null and Return Data`() = runTest {
        if (::retrofitRepository.isInitialized) {
        val dummyStory = DataDummy.generateDummyStoryResponse()
        val data: PagingData<ListStoryItem> = StoryPagingSource.snapshot(dummyStory)
        val expectedStory = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStory.value = data
        Mockito.`when`(retrofitRepository.getStories()).thenReturn(expectedStory)


        val mainViewModel = MainViewModel(retrofitRepository)
        val actualStory: PagingData<ListStoryItem> = mainViewModel.storyResponseLiveData.getOrAwaitValue()
